...
akka is a platform for building simple, correct, concurrent, and scalable applications for the jvm.  This module allows you to configure akka through The Play! framework's conf/application.conf file, provides a few command-line scripts to start akka when your application starts, and has some examples of it in action.
...
There are two examples included with the module, located in the sample-app folder.  To run the sample-app, make sure scala-head is installed, cd to the sample-app directory and execute:
